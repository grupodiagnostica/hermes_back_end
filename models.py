...
class Paciente(db.Model):
    ...
    diagnosticos = db.relationship('Diagnostico', backref='pessoa', lazy=True)
    def __init__(self, id_pessoa, sexo, tipo_sanguineo, detalhes_clinicos, logradouro, bairro, cidade, numero, estado, id=None):
        if id is None:
            self.id = str(uuid.uuid4())
        else:
            self.id = id
